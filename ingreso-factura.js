...
async function guardarFacturaIngreso() {
    try {
        ...
        // Calcular total
        const totalFactura = productos.reduce((sum, p) => sum + p.subtotal, 0);
        ...
        try {
            ...
            if (updatedLines.length > 0) {
                updatedLines.forEach(line => {
                    // line formato: "codigo: prev → nuevo"
            ...
            if (insertedLines.length > 0) {
                insertedLines.forEach(line => {
                    // line formato: "codigo - nombre (stock X)"
            ...
            await proveedoresAlert('Factura guardada exitosamente', 'success', '¡Éxito!', detallesHtml);
        } catch (errAlert) {
            await proveedoresAlert('Factura guardada exitosamente', 'success', '¡Éxito!');
        }

        // Reiniciar y volver al modo facturas (harcodear en bloque seguro)
        setTimeout(() => {
            try {
                ingresoFacturaState = {
                    pasoActual: 1,
                ...
                // Limpiar campos del formulario para evitar renders sobre DOM inexistente
                ...
                // En lugar de cerrar el módulo, llevar al usuario a la selección de proveedor (Paso 1)
                try {
                    // Asegurar que no haya proveedor seleccionado
                    ingresoFacturaState.proveedorSeleccionado = null;

                    // Limpiar selección visual de botones de proveedor si existen
                    document.querySelectorAll('.proveedor-btn-ingreso.selected').forEach(b => b.classList.remove('selected'));

                    // Mostrar paso 1 (selección de proveedor) y recargar la lista
                    if (typeof mostrarPasoIngreso === 'function') {
                        mostrarPasoIngreso(1);
                    }

                    if (typeof cargarProveedoresIngreso === 'function') {
                        cargarProveedoresIngreso();
                    }

                    // Poner foco en el grid o en el campo de búsqueda si existe
                    const busqueda = document.getElementById('busquedaProveedorIngreso') || document.getElementById('busquedaProductoIngreso');
                    if (busqueda) {
                        busqueda.focus();
                    }
                } catch (errInner) {
        ...
        hideAppLoader();
        await proveedoresAlert('Error al guardar la factura: ' + error.message, 'error', 'Error');
    }
}
...
async function procesarProductosNuevos(productos) {
    ...
    // 3) Aplicar incrementos de stock en batch vía RPC (si existe), o fallback paralelo
    if (stockIncrements.length > 0) {
        try {
            // Intentar RPC 'increment_stock' que acepte un array de {codigo, cantidad}
            if (client && typeof client.rpc === 'function') {
                try {
                    const { data: rpcData, error: rpcErr } = await client.rpc('increment_stock', { items: stockIncrements });
                    if (rpcErr) throw rpcErr;
                    // rpcData podría contener los rows actualizados; usarlo para rellenar resultados finales
                    if (Array.isArray(rpcData)) {
                        rpcData.forEach(r => {
                            const idx = results.findIndex(x => x.codigo === (r.codigo || r.codigo));
                            if (idx !== -1) {
                                // Si la entrada era 'to_update', convertir a 'updated'
                                if (results[idx].action === 'to_update') {
                                    results[idx] = { codigo: r.codigo, action: 'updated', previousStock: results[idx].previousStock, nuevoStock: r.stock };
                                }
                            }
                        });
                    }
            ...
            try {
                ...
                await Promise.all(stockIncrements.map(async si => {
                    ...
                    try {
                        const done = stockIncrementsProcessed ? stockIncrementsProcessed + 1 : 1;
